<template>
  <div class="app-container">
    ...
    <el-dialog :visible.sync="dialogVisible" :title="dialogTitle" width="600px">
      <el-form ref="notificationForm" :model="temp" :rules="notificationRules" label-width="100px" style="width: 400px" label-position="left">
        ...
        <el-form-item label="开始时间" prop="startTime">
          <el-date-picker type="datetime" v-model="temp.startTime" placeholder="发布时间" value-format="yyyy-mm-dd HH:mm:ss" format="yyyy年MM月dd日 HH时mm分ss秒"/>
        </el-form-item>
        <el-form-item label="结束时间" prop="endTime">
          <el-date-picker type="datetime" v-model="temp.endTime" placeholder="发布时间" value-format="yyyy-mm-dd HH:mm:ss" format="yyyy年MM月dd日 HH时mm分ss秒"/>
        </el-form-item>
  ...
  methods: {
    ...
    updateD(row) {
      ...
      this.$refs['notificationForm'].validate().then(valid => {
        updateNotification(this.temp).then(res => {
          if (res.status) {
    ...
    createD() {
      this.$refs['notificationForm'].validate().then(valid => {
        createNotification(this.temp).then(res => {
          if (res.status) {
            this.dialogVisible = false
            this.$notify({
              title: '成功',
    ...
    handleDelete(row) {
      this.$confirm('确认删除此这个文章吗？', '确认', {
        ...
        deleteNotification({ 'notificationId': row.id }).then(res => {
          if (res.status) {
    ...
    redictToNotificationUser(row) {
      this.$router.push({ name: 'notificationUser', params: { id: row.id }})
    }
  }
}
</script>
